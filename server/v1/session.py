...
from .ai import ask
from .db import Database, User, Session, Chat, ChatResponse
...
@router.get("/new")
async def new_session(name: str, user: User = Depends(manager)) -> dict:
    async with Database.async_session() as session:
        session.add(Session(username=user.username, session_name=name))
